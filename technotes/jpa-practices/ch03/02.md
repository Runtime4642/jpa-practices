...
1.  me.kickscar.practices.jpa.ch03._02.domain.Guestbook.java Entity Mapping 참고


### 2. Implementation & Testing Repository

#### 2-1. Tech. Points
1. JPQL, QueryDSL 그리고 Spring Data JPA 기반의 개별적인 Repository Implementation과 Configuration을 이해한다.
2. Repository에서 Persistent Object를 다루는 방법과 Transaction과의 관계를 이해한다.
...
#### 2-3. JpqlGuestbookRepository Test : Repository Based On JPQL
1. JpqlConfig.java    
    ...
    6. JPA Properties(ch02의 application.yml의 JPA 섹션을 참고하면 쉽게 이해할 수 있다.)
...
2. JPA 트랜잭션 관리에 관해서[중요개념]    
    ...
    2. Spring Conatiner 에서의 트랜잭션과 영속성컨텍스트
        ...
        - 반대로, 여러 요청에 대한 여러 쓰레드에서 같은 엔티티매니저를 쓰는 경우는 Spring MVC에서는 흔한 일이다. 이런 경우는 영속성컨텍스트가 다르기 때문에 멀티쓰레드에서는 안전하다.
        - Spring MVC에서는 비즈니스 로직을 시작하는 서비스 계층에서 AOP가 적용된 @Transactional으로 트랜잭션을 시작하는 것이 보통이다.
    3. 결론은 Spring Container가 알아서 쓰레드와 연관된 트랜잭션과 영속성켄텍스트의 관리를 하기 때문에 개발자는 @Transactional과 비즈니스 로직에만 집중하면 된다.
    4. 이 내용을 숙지하지 못하면 지연로딩이나 프록시초기화 등의 문제가 발생할 경우, 해결에 많은 노력을 기울여야 하기 때문에 꼭 이해해야 할 개념이다.
...
1. JpaConfig.java    
    ...
    3. JPA Repositories 활성화: 애플리케이션 실행 시, Spring Data JPA가 JpaRepository 인터페이스를 상속받은 Repository 인터페이스 구현체를 자동으로 생성 하게끔 한다. 
       ...
       따로 이 레포지토리 구현체에 대한 컴포넌트 스캔을 하지 않도록 한다. 경우에 따라 빈 등록 중복 예외가 발생할 수 있다.
