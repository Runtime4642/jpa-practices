...
4. __JpaBoardRepositoryTest.java__
    1) test01Save
        ...
        - 실제 코드는 기본 메소드 save(entity)를 오버로딩한 JpaCommentQryDslRepositoryImp.save(boardNo, comments) 이다.  
            ...
            4) 실행된 Update 쿼리 로그    
                ...
                ```
                    Hibernate: 
                    /* create one-to-many row me.kickscar.practices.jpa.ch03._05.domain.Board.comments */ update
                        comment 
                    set
                        board_no=? 
                    where
    ...
    4) test04BoardListLazyProblemSolved()
        ...
        - 일단은 JPQL(QueryDSL)로 해결할 수 있다. 
            1) QueryDSL 경우에는 innerJoin(), fetchJoin()를 사용해 개선할 수 있다(ch03/02 참고)
            ...
            5) 하지만, QueryDSL에서는 innerJoin() 만으로도 join fetch으로 생성된 쿼리와 같은 쿼리 실행이 가능하다.(ch03/02 참고)
        - 테스트 실행과 쿼리 로그 
